...
class AffineNode(object):
    ...
    def backward(self):
        d_W = np.outer(self.d_out, np.transpose(self.x.out))
        d_x = np.dot(np.transpose(self.W.out), self.d_out)
        d_b = self.d_out * 1
        self.W.d_out += d_W
        self.x.d_out += d_x
        self.b.d_out += d_b
...
class SoftmaxNode(object):
    ...
    def __init__(self, z, node_name):
        ...
        self.z = z
    def forward(self):
        self.divider = np.sum(np.exp(self.z.out))
        self.out = np.exp(self.z.out)/self.divider
        self.d_out = np.zeros(self.out.shape)
        return self.out
    def backward(self):
        K = len(self.z.out)
        derivative = np.empty([K, K])
        for i in range(K):
            for j in range(K):
                if i == j:
