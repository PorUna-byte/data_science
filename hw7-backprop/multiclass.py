...
class MulticlassClassifier(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 1000, num_class=3):
        ...
        self.L = nodes.AffineNode(self.W1, self.x, self.b1, "Hidden unit")
        self.h = nodes.TanhNode(self.L, "tanh")
        self.scores = nodes.AffineNode(self.W2, self.h, self.b2, "scores unit")
        self.prediction = nodes.SoftmaxNode(self.scores, "softmax")
        self.objective = nodes.NLLNode(self.prediction, self.y, "objective")
