...
class LassoRegression(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y=None):
        ...
        # now perform shooting algorithm to calculate w
        # w starts from the ridge regression solution suggested by Murphy
        # w = np.linalg.inv(X.T @ X + self.l1reg * np.identity(num_ftrs)) @ X.T @ y
        w = np.zeros(num_ftrs)
        for i in range(10000):
            j = i % num_ftrs  # cyclic coordinate descent
            a_j = 2 * np.dot(X[:, j].T, X[:, j])
            c_j = 2 * np.dot(X[:, j].T, y - X @ w + w[j] * X[:, j])
            if c_j < - self.l1reg:
...
def compare_our_lasso_with_sklearn(X_train, y_train, l1_reg=1):
    ...
    # Now run our ridge regression and compare the coefficients to sklearn's
    lasso_regression_estimator = LassoRegression(l1reg=l1_reg)
...
def do_grid_search_lasso(X_train, y_train, X_val, y_val):
    ...
    # Now we set up and do the grid search over l1reg. The np.concatenate
    # command illustrates my search for the best hyperparameter. In each line,
    # I'm zooming in to a particular hyperparameter range that showed promise
    # in the previous grid. This approach works reasonably well when
    # performance is convex as a function of the hyperparameter, which it seems
...
def main():
    ...
    # Compare our LassoRegression to sklearn's.
    # compare_our_lasso_with_sklearn(X_train, y_train, l1_reg=1)
    # Do hyperparameter tuning with our ridge regression

    grid_2, results_2 = do_grid_search_ridge(X_train, y_train, X_val, y_val)
    print(results_2)

    grid_1, results_1 = do_grid_search_lasso(X_train, y_train, X_val, y_val)
    print(results_1)

    # Plot validation performance vs regularization parameter
    fig, ax = plt.subplots()
    #    ax.loglog(results["param_l2reg"], results["mean_test_score"])
    ax.semilogx(results_2["param_l2reg"], results_2["mean_test_score"])
    ax.grid()
    ...
    fig, ax = plt.subplots()
    #    ax.loglog(results["param_l2reg"], results["mean_test_score"])
    ax.semilogx(results_1["param_l1reg"], results_1["mean_test_score"])
    ax.grid()
    ...
    l2regs = [0, grid_2.best_params_['l2reg'], 1]
    l1regs = [0, grid_1.best_params_['l1reg'], 1]
    X = featurize(x)
    # for l2reg in l2regs:
